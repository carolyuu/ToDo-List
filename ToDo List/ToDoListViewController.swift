...
class ToDoListViewController: UIViewController {
    ...
    func authorizeLocalNotifications () {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert,.sound,.badge]) { (granted, error) in
            ...
            if granted {
                ...
                //TODO: put in alert telling user whatto do
            }
        }
    }
    func setNotifications() {
        ...
        //and lets recreate them with updated data we just saved
...
extension ToDoListViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print("numberOfRowsInSection was just called. Return \(toDoItems.count)")
        return toDoItems.count
    }
    
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        print("cellForRowAt was jsut called for indexPath.row = \(indexPath.row) which is the cell containing \(toDoItems[indexPath.row])")
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) // set upreusable cell
        cell.textLabel?.text = toDoItems[indexPath.row].name // put data inside label in cell
        return cell
    }
